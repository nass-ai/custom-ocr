...
def smoothen_binarize_and_dilate(raw_image):
...
def smoothen_and_binarize(raw_image):
...
def getTransformationMatrix(bw_img):

    #Find all white pixels
    pts = np.empty([0,0])
    pts = cv2.findNonZero(bw_img)
    ...
    # Deskew(Rotate) the image according to the found angle
